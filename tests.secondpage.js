casper.test.begin('Second Page Tests', 11, function suite(test) {
  casper.start('http://brechy.net/index1', function() {
    ...
    test.assertType('h3', 'string', '06 - text is formated as a string')
    ...
    test.assertExists('gcse:search', '11 - Google search engine exists');
  });
  casper.run(function() {
    test.done();
  });
