...
casper.test.begin('Testing navigation and button clicks', 4, function suite(test) {
  ...
  casper.then(function () {
    ...
    test.assertUrlMatch(/\/\/brechy\.net\index1/, 'New location is ' + this.getCurrentUrl());

    // Report that we're attempting to use keyboard nav.
    test.comment('⌚️  Using mouse to click link...');
  ...
  casper.then(function () {
    ...
    test.assertUrlMatch(/Explore/, 'New location is ' + this.getCurrentUrl());
